...
const {
  data: nextSeriesCourse,
} = await supabase
  .from("series_courses")
  .select("course_id")
  .eq(
    "series_id",
    currentSeriesCourse.series_id,
  )
  .gt(
    "order_index",
    currentSeriesCourse.order_index,
  )
  .order("order_index", {
    ascending: true,
  })
...
if (nextSeriesCourse) {
  const { data: nextCourse } =
    await supabase
      .from("courses")
      .select("id")
      .eq(
        "id",
        nextSeriesCourse.course_id,
      )
...
return ( <main className="min-h-screen bg-background px-6 py-10"> <div className="mx-auto max-w-6xl">
